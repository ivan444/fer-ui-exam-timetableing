...
/**
 * Postupak mutacije zamjenom jednog gena
 ...
 * 
 */
public class PointMutation extends Mutator {

	public PointMutation(float mutationFactor, TermBean[] allTerms) {
	...
	public void mutate(Individual individual) {
		...
		if (Math.random() < mutationFactor) {
			int orgIndex = (int) Math.round(Math.random()*(terms.length-1));
			int swpIndex = (int) Math.round(Math.random()*(allTerms.length-1));
			individual.setTerm(orgIndex, allTerms[swpIndex]);
		}
	}
}
